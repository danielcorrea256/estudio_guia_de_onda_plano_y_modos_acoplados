...
def funcion_rayo(n_co, n_t, h, k_0, m, modo):

    #función phi que depende del modo (TM o TE)
    Phi = phi(n_co, n_t, modo) 
    
    # La ecuación que queremos resolver
    z = lambda theta: 2 * n_co * k_0 * h*math.pi*math.cos(theta) - (4 * Phi(theta)) - (2 * m * math.pi)

    return z  # Buscamos la raíz de esta función

def metodo_rayo(n_co, n_t, h, k_0, ms):
    TEs = {}
    TMs = {}

    # theta critico
    theta_c = math.asin(n_t/n_co)
    for m in ms:
        # resultado para modo TE
        f_TE = funcion_rayo(n_co=n_co,n_t=n_t,h=h,k_0=k_0,m=m, modo='TE')
        theta_TE = root_scalar(f_TE,method="bisect",bracket=[theta_c,math.pi/2]).root
        TEs[m] = math.degrees(theta_TE)

        # resultado para modo TM
        f_TM= funcion_rayo(n_co=n_co,n_t=n_t,h=h,k_0=k_0,m=m, modo='TM')
